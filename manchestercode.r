...
g1 <- ggplot(spatialDist, aes(x=distance, y=0, col=site)) + xlim(c(0, 100)) + geom_segment(col='grey40', x=0, xend=max(spatialDist$distance), y=0, yend=0) + geom_point()  + geom_text(aes(label=site), vjust=-1)  + theme_bw() + theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), panel.grid.major=element_blank(), panel.grid.minor=element_blank(), axis.title.y=element_blank()) + xlab('distance (km)') + scale_color_manual(values=c("skyblue3","goldenrod1","indianred2","palegreen4", "darkorange3")) + theme(legend.position='none') + ylim(c(-0.05,0.1))
...
g2 <- ggplot(pcaLogValues, aes(y=Comp.1, x=Comp.2, col=site)) + geom_density2d(aes(col=site), alpha=0.2) + geom_point() + facet_wrap(~site, ncol=5) + theme_bw() + theme(legend.position='none') + scale_color_manual(values=c("skyblue3","goldenrod1","indianred2","palegreen4", "darkorange3"))


# final composition

svg('pcs.svg', width=25, height=5)    
